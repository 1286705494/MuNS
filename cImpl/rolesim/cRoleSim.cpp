...
//                    float *mIn = new float[vInDeg[i] * vInDeg[j]];
                	...
                	int x = 0, y = 0;
                    for (typename set<int>::const_iterator xit = vvNeigh[i].begin(); xit != vvNeigh[i].end();
                    		++xit, ++x)
                    {
                    	y = 0;
                        for (typename set<int>::const_iterator yit = vvNeigh[j].begin(); yit != vvNeigh[j].end();
                        		++yit, ++y)
                        {
